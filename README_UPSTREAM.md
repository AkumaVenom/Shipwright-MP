...
# Custom direct-IP multiplayer source preview

This modified baseline adds **Network → Multiplayer → Host & Play / Join by IP**, with personal saves by default and opt-in shared adventure progression. Read [MULTIPLAYER_QUICKSTART.md](MULTIPLAYER_QUICKSTART.md) before building or testing. This archive is source-only; the game integration is not yet full-build/two-PC validated, and enemy/boss/physics/cutscene authority is not implemented. See the [change log](CHANGELOG_MULTIPLAYER.md) and [validation record](docs/multiplayer/VALIDATION.md). The upstream instructions below remain for reference; downloading an ordinary upstream release will not include this custom implementation.
